...
import Hero from "@/app/hero/page"; // Importa el Hero desde la carpeta correcta
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 container py-8">
        <h1 className="text-3xl font-bold align-content: center;">Our Luxury Fleet</h1>
        <p className="mt-4">Choose from our selection of premium yachts for your perfect day on the water</p>
          ...
          {products.map((product) => (
            <div key={product.id} className="border p-4 rounded-lg shadow-lg">
              ...
              <div className="flex items-center">
                {/* Muestra las estrellas completas */}
                {[...Array(Math.floor(product.rating ?? 0))].map((_, index) => (
                  <Star key={index} className="h-4 w-4 fill-yellow-400 text-yellow-400" />
                ))}
                
                {/* Muestra una estrella media si el rating es decimal */}
                ...
                {/* Rellena el resto con estrellas vacías */}
      ...
      <Services services={products.map(product => ({
        id: product.id.toString(),
        icon: <Star className="h-4 w-4" />, // Example icon, replace with actual icon if needed
        name: product.name,
